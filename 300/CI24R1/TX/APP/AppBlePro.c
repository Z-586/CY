...
static uint16_t count_delay_time1 = 0;
...
static Ci24r1Mode_e CI24R1_Mode = MODE_TX;
...
void switch_Mode(Ci24r1Mode_e Mode){
	if(Mode == MODE_RX){
		DrvCi24r1Init(MODE_TX);
		CI24R1_Mode = MODE_TX;
	}else{
		DrvCi24r1Init(MODE_RX);
...
{	
	m_parse_date_timer = xTimerCreate("parse_date", 200, pdTRUE, NULL, parse_date_handler);
	if(pdPASS != xTimerStart(m_parse_date_timer, 0)){}
	DrvCi24r1Init(MODE_TX);//ci24r1切换为发送模式	
	vTaskDelay(200);
	PwmFreq = Set_140K;
	while(1){
		if(pwm_start_flag == 0){
				...
				if(count_time % 500 == 0){
					if(PwmFreq < Set_100K){
						...
						if(count_delay_time % 200 == 0){
							count_time = 1;
							count_delay_time = 500;
							count_delay_time1 = 0;
							HAL_HRTIM_WaveformCounterStop(&hhrtim1, HRTIM_TIMERID_TIMER_A);
						}								
					}			
				}
				if(count_delay_time != 500 ){
					if((count_delay_time1 % 100) == 99 || (count_delay_time % 50) == 49)
						switch_Mode(CI24R1_Mode);	
					if(CI24R1_Mode == MODE_TX){
						tx_status = DrvCi24r1Send(tx_buf,10);	
						if(tx_status == TX_OK)  //模块发送数据并判断是否发送成功
						{
							DrvCi24r1Init(MODE_RX);
							Ci24R1_Write_Cmd(SELIRQ);
							CI24R1_DATA_IN();
						...
						rx_status = Ci24R1RxPacket(rx_buf);
						if(rx_status == 0){
							TX_RX_status = 1;
							g_tx_system_stat = STATE_FLAGE_CHARGING;
							vTaskDelay(10);
