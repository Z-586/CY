...
uint8_t	  RS485_Addr          = 0xff;
uint8_t   baud                = 0;

uint32_t  DATA_DataTab[8] ={0};
uint32_t  DATA_StartAddr = 0x1C000000;		
uint32_t  DATA_StopAddr = 0x1C0003FF;
volatile uint32_t U_TxdFlag =0;

#define FLASH_SAVE_ADDR  0x1C000000  //0X08020000 	//设置FLASH 保存地址(必须为偶数，且其值要大于本代码所占用FLASH的大小+0X08000000)
...
void SoftReset(void)
{
	NVIC_DisableIRQ(UART1_IRQn); // 关闭所有中端
	NVIC_DisableIRQ(ACMP_IRQn);
	NVIC_SystemReset(); // 复位
}

void USART2_CommSendAtStr(uint8_t * pArray, uint32_t len){
	U_TxdFlag = 0;
	for(int i=0;i<len;i++)
	{
		UART_WriteTHR(UART1,(uint8_t)(*pArray++));
		if(i % 15 == 0){
			while( !U_TxdFlag);
			U_TxdFlag = 0;
		}
...
void STMFLASH_Read(u32 ReadAddr,u16 *pBuffer,u16 NumToRead)   	
{
	...
	u32 Date_buff[8] = {0};//暂定为8
	for(i = 0; i < Lenth; i++)
	{
		Date_buff[i] = FMC_Read(ReadAddr);			    //读取4个字节.
		ReadAddr += 4;									//偏移4个字节.	
...
void IN16_TO_IN32(u16* Buff,u16 Len){//目前暂定 可以写32字节数据
	uint8_t Buf_Len = (uint8_t)((Len+1)/2);
	for(int i = 0; i < Buf_Len; i++){
		DATA_DataTab[i] =((u32)Buff[i*2 + 1] << 16) |(u32)Buff[i*2] ;
	}
}


void STMFLASH_Write(u32 ReadAddr,u16 *pBuffer,u16 NumToRead)   	
{
	uint8_t Buf_Lenth = (uint8_t)((NumToRead+1)/2);
	IN16_TO_IN32(pBuffer,NumToRead);
	FMC_ErasePage(DATA_StartAddr);
	FMC_ErasePage(DATA_StartAddr);
	for(int i = 0; i<Buf_Lenth ; i++)
		FMC_Write(DATA_StartAddr + i*4 , DATA_DataTab[i]);	
}
	...
	if (Buffer_Rx.buf[0] == RS485_Addr) { //
		if((Buffer_Rx.buf[1] == 03) || (Buffer_Rx.buf[1] == 06)) {
			...
			if(startRegAddr < 1000) {
				calCRC = usCRC16(Buffer_Rx.buf, RS485_Lenth-2);//
				recCRC = Buffer_Rx.buf[RS485_Lenth-2]|(((u16)Buffer_Rx.buf[RS485_Lenth-1])<<8);//
...
{			
	Modbus_RegMap();
	UART_UART1_Config(Setting_baud(baud));
}
