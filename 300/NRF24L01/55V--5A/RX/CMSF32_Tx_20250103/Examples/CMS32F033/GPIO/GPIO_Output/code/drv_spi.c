...
void drv_spi_init( void )
{
	/** SPI引脚配置 */
	//SCK MOSI NSS配置为推挽输出
	SYS_SET_IOCFG(IOP07CFG,SYS_IOCFG_P07_GPIO);				//初始化SCK P07
	GPIO_CONFIG_IO_MODE(SPI_CLK_GPIO_PORT,SPI_CLK_GPIO_PIN,GPIO_MODE_OUTPUT);
	GPIO0->DO_f.P7 = 0;										//初始化状态设置为低	
	
	SYS_SET_IOCFG(IOP05CFG,SYS_IOCFG_P05_GPIO);				//初始化MOSI P05	
	GPIO_CONFIG_IO_MODE(SPI_MOSI_GPIO_PORT,SPI_MOSI_GPIO_PIN,GPIO_MODE_OUTPUT);
	GPIO0->DO_f.P5 = 1;										//初始化状态设置为高
	
	SYS_SET_IOCFG(IOP04CFG,SYS_IOCFG_P04_GPIO);				//初始化NSS P04	
	GPIO_CONFIG_IO_MODE(SPI_NSS_GPIO_PORT,SPI_NSS_GPIO_PIN,GPIO_MODE_OUTPUT);
	GPIO0->DO_f.P4 = 1;										//初始化状态设置为高
	
	//初始化MISO 上拉输入
	SYS_SET_IOCFG(IOP06CFG,SYS_IOCFG_P06_GPIO);				//初始化MISO P06		
	GPIO_CONFIG_IO_MODE(SPI_MISO_GPIO_CLK,SPI_MISO_GPIO_PIN,GPIO_MODE_INPUT_WITH_PULL_UP);		
}
