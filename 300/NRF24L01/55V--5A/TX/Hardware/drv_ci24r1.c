...
DrvCi24r1Init(MODE_TX);//设置RF为发送模式

***********RX************

DrvCi24r1Init(MODE_RX);//设置RF为接收模式
Ci24R1_Write_Cmd(SELIRQ);//选择DATA引脚输出IRQ值
CI24R1_DATA_IN();

************************/
	
static const unsigned char Iint_ADDR[ADR_WIDTH]= {0x00,0x1A,0xB1,0x21,0x01}; // //默认节点地址
...
uint8_t DrvCi24r1Init( Ci24r1Mode_e Mode )
{
	...
	if(DrvCi24r1IsExist())
	{
		...
		if(Mode == MODE_TX){
			Ci24R1_Write_Buf(W_REGISTER+RX_ADDR_P0,(uint8_t *)Iint_ADDR,ADR_WIDTH);	//写入RX节点地址
			Ci24R1_Write_Buf(W_REGISTER+TX_ADDR,(uint8_t *)Iint_ADDR,ADR_WIDTH); //写入TX节点地址
		}
		else{
