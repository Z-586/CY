...
void drv_spi_init( void )
{
	GPIO_InitTypeDef	SpiGpioInitStructer = {0};
	
	/** SPI引脚配置 */
	__HAL_RCC_GPIOB_CLK_ENABLE();
	
	//SCK MOSI NSS配置为推挽输出
	HAL_GPIO_WritePin(SPI_CLK_GPIO_PORT,  SPI_CLK_GPIO_PIN, GPIO_PIN_RESET);
	...
	SpiGpioInitStructer.Pin =SPI_NSS_GPIO_PIN|SPI_CLK_GPIO_PIN|SPI_MOSI_GPIO_PIN;
	SpiGpioInitStructer.Mode = GPIO_MODE_OUTPUT_PP;
	SpiGpioInitStructer.Pull = GPIO_PULLUP;
	SpiGpioInitStructer.Speed = GPIO_SPEED_FREQ_LOW;
	HAL_GPIO_Init(SPI_CLK_GPIO_PORT, &SpiGpioInitStructer);
	
	//初始化MISO 上拉输入	
	SpiGpioInitStructer.Pin = SPI_MISO_GPIO_PIN;
	SpiGpioInitStructer.Mode = GPIO_MODE_INPUT;
	SpiGpioInitStructer.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(SPI_MISO_GPIO_PORT, &SpiGpioInitStructer);
