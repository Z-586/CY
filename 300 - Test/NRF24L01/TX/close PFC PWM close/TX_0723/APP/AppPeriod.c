...
void Check_TemperatureCurrent_Status(void)
{
	...
	if (TXparameter.TX_T1 >= TX_TEMP_OPEN_fAN) {
		...
		__HAL_TIM_SetCompare(&htim8,TIM_CHANNEL_1,6800);
	}		
}
...
//	FanCtrl_Handle           = xTimerCreate("Fan Control",          2000,  pdFALSE, NULL, FanCtrlCallback); 
	...
	while (1)
	{
		...
		TX_TEMPER_1();        //采集温度1
		Adc_deal();//
		if ((TXparameter.Tx_U_PFC >= 0) && (TXparameter.Tx_U_PFC <= 410000)) {
			_ready1 = HAL_GPIO_ReadPin(FLAG_READY_GPIO_Port, FLAG_READY_Pin);
			_fault1 = HAL_GPIO_ReadPin(FLAG_FAULT_GPIO_Port, FLAG_FAULT_Pin);
			...
			Debug_Printf("TXparameter.Tx_U_PFC：%d \r\n",TXparameter.Tx_U_PFC);
		}
		vTaskDelay(100);
	}
}
